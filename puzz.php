<?php
			...
			$sql2 = "SELECT count(*) as c from `maps` WHERE category = 'puzzle'"; //atiras
			...
			$sql = "SELECT  `name`, `description`, `pic`, `size`, `downloadlink`, `downloads`, `author` FROM `maps` WHERE category = 'puzzle' ORDER BY time desc limit ".$oldal * $peroldal.",".$peroldal; //atiras
			...
			echo '<br/><hr/>';
			echo '<p class="iras6">Puzzle Maps</p>'; //atiras
			...
			if ($oldal + 1 != 1)
		echo  ' <td align="center"><a href="index.php?menu=categories&amp;amenu=puzz&amp;page='.($oldal).'" class="glink">Previous</a></td>';//ai
		
		for ($i = 1; $i <= $oldalakszama; $i++)
		{
			echo ' <td align="center"><a href="index.php?menu=categories&amp;amenu=puzz&amp;page='.$i.'"> | '.$i.' | </a></td> ';//ai
		} 
		
		
		
		if ($oldal + 1 != $oldalakszama)
		echo  ' <td align="center"><a href="index.php?menu=categories&amp;amenu=puzz&amp;page='.($oldal + 2).'" class="glink">Next</a></td>';//ai
