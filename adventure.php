<?php
			...
			$sql = "SELECT  `name`, `description`, `pic`, `size`, `downloadlink`, `downloads`, `author` FROM `maps` WHERE category = 'adventure' ORDER BY time desc limit ".$oldal * $peroldal.",".$peroldal;
			...
			echo '<br/><hr/>';
			echo '<p class="iras6">Adventure Maps</p>';
			...
			if ($oldal + 1 != 1)
		echo  ' <td align="center"><a href="index.php?menu=categories&amp;amenu=adv&amp;page='.($oldal).'" class="glink">Previous</a></td>';
		
		for ($i = 1; $i <= $oldalakszama; $i++)
		{
			echo ' <td align="center"><a href="index.php?menu=categories&amp;amenu=adv&amp;page='.$i.'" > | '.$i.' | </a></td> ';
		} 
		
		
		
		if ($oldal + 1 != $oldalakszama)
		echo  ' <td align="center"><a href="index.php?menu=categories&amp;amenu=adv&amp;page='.($oldal + 2).'" class="glink">Next</a></td>';
